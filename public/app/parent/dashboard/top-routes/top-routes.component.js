(function() {
  "use strict";
  angular.module("app").component("topRoutes", {
  ...
  function controller($stateParams, $state) {
    ...
    vm.$onInit = function() {
      ...
      function convertDataCountObjForD3(
        dataOccurances,
        propertyBeingCounted,
        resultPropertyName
      ) {
        var d3DataArray = [];
      ...
      /////// CREATE TOOLTIP
      var topRoutesTooltip = d3
        .select("body")
        .append("div")
        .attr("class", "tooltip")
        .attr("id", "top-routes-tooltip");

      /////// REQUEST DATA
      d3.xhr("/logs/top-routes", function(err, data) {
        ////// PREP REQUESTED DATA FOR D3
        var d3RouteData = convertDataCountObjForD3(
          countDataOccurances(data, "url"),
          "url",
          "count"
        )
          .sort(function(x, y) {
        ...
        var arcOver = d3.svg.arc().outerRadius(outerRadius + 18);
        // .innerRadius(innerRadius)
